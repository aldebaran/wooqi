...
from wooqi.pytest._pytest._code.code import TerminalRepr
from wooqi.pytest._pytest.compat import (
...
from wooqi.pytest._pytest.outcomes import fail, TEST_OUTCOME
from wooqi.pytest._pytest.compat import FuncargnamesCompatAttr
...
def pytest_sessionstart(session):
    import wooqi.pytest._pytest.python
    scopename2class.update({
        'class': wooqi.pytest._pytest.python.Class,
        'module': wooqi.pytest._pytest.python.Module,
        'function': wooqi.pytest._pytest.main.Item,
    })
...
def add_funcarg_pseudo_fixture_def(collector, metafunc, fixturemanager):
    ...
    for argname, valuelist in arg2params.items():
        ...
        if scope != "function":
            ...
            if node is None:
                assert scope == "class" and isinstance(
                    collector, wooqi.pytest._pytest.python.Module)
...
class FixtureRequest(FuncargnamesCompatAttr):
    ...
    def instance(self):
        ...
        # unittest support hack, see wooqi.pytest._pytest.unittest.TestCaseFunction
        try:
...
def fail_fixturefunc(fixturefunc, msg):
    ...
    source = wooqi.pytest._pytest._code.Source(fixturefunc)
    fail(msg + ":\n\n" + str(source.indent()) + "\n" + location,
         pytrace=False)
    ...
    def pytest_generate_tests(self, metafunc):
        for argname in metafunc.fixturenames:
            ...
            if faclist:
                ...
                if fixturedef.params is not None:
                    func_params = getattr(
                        getattr(metafunc.function, 'parametrize', None), 'args', [[None]])
